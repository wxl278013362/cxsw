...
S32 GcodeM869Parse(const S8 *CmdBuff, VOID *Arg)
{
    ...
    S32 Offset = strlen(CMD_M869) + 1;
    S32 Len = strlen(CmdBuff);
    if ( strncmp(CmdBuff, CMD_M869, strlen(CMD_M869)) != 0 )
    {
        return CrErr;
    }

    struct ArgM869_t *CmdArg = (struct ArgM869_t *)Arg;
    while ( Offset < Len )
    {
        S8 Opt[20] = {0};
        ...
        if ( OptLen <= 1 )
        {
            printf("Gcode M869 has no param opt = %s\n", Opt);
        }
        ...
        switch ( Opt[0] )  //判断命令
        {
            ...
            default:
                printf("Gcode M869 has no define opt = %s\n", CmdBuff);
                break;
        }
...
S32 GcodeM869Exec(VOID *ArgPtr, VOID *ReplyResult)
{
    ...
    struct ArgM869_t *Param = (struct ArgM869_t *)ArgPtr;
