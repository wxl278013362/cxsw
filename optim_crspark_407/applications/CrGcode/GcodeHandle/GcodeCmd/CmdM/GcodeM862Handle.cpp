...
S32 GcodeM862Parse(const S8 *CmdBuff, VOID *Arg)
{
    ...
    S32 Offset = strlen(CMD_M862) + 1;
    S32 Len = strlen(CmdBuff);
    if ( strncmp(CmdBuff, CMD_M862, strlen(CMD_M862)) != 0 )
    {
        return CrErr;
    }

    struct ArgM862_t *CmdArg = (struct ArgM862_t *)Arg;
    while ( Offset < Len )
    {
        S8 Opt[20] = {0};
        ...
        if ( OptLen <= 1 )
        {
            printf("Gcode M862 has no param opt = %s\n", Opt);
        }
        ...
        switch ( Opt[0] )  //判断命令
        {
            ...
            default:
                printf("Gcode M862 has no define opt = %s\n", CmdBuff);
                break;
        }
...
S32 GcodeM862Exec(VOID *ArgPtr, VOID *ReplyResult)
{
    ...
    struct ArgM862_t *Param = (struct ArgM862_t *)ArgPtr;
