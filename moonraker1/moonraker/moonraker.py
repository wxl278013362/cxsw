...
def offset_to_number(offset, probe_count, mesh_min, mesh_max, bltouch_x=0.0, bltouch_y=0.0):
    ...
    line_count, row_count = probe_count.split(',')
    ...
    unit_x = float((max_x-min_x)/(line_count-1))
    unit_y = float((max_y-min_y)/(row_count-1))
    print(min_x, min_y, max_x, max_y, unit_x, unit_y)
    for i in range(line_count):
        # 当x,y都处于最小坐标时, 返回最小编号 1
        if float(x) <= min_x + 1 and float(y) <= min_y + 1:
            return 1
        if float(x) <= min_x + i * unit_x:
            for j in range(row_count):
                if float(y) <= unit_y*j + min_y + 1:
                    if j > 0:
                        # 偶数列处理, 从大到小
                        if (j + 1) % 2 == 0:
                            # 行数乘以列数
                            return j * row_count + (line_count - i)
                        # 奇数列处理, 从小到大
                        else:
                            # 行数乘以列数, 4 * 5 + (4 + 1)
                            return j * row_count + (i + 1)
                    else:
                        return i + 1
...
def read_config():
    try:
        ...
        # path = r"C:\Users\102393\Desktop\临时\printer.cfg"
        try:
